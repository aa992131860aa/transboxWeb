...
(function (a) {
    ...
    v += '           {{#url}}<div class="zeromodal-loading1"></div>{{#iframe}}<iframe zero-unique-frame="{{unique}}" frameborder="0"   scrolling="auto"  src="{{url}}" class="zeromodal-frame"></iframe>{{/iframe}}{{/url}}';
    v += "       </div>";
    ...
    z.alert = function (C) {
        var B = {iconClass: "show-zero2 zeromodal-icon-info", iconText: "!"};
        var D = {};
        f.extend(D, B);
        if (typeof C === "object") {
            f.extend(D, C)
    ...
    z.error = function (B) {
        var C = {iconDisplay: '<div class="show-zero2 zeromodal-icon zeromodal-error"><span class="x-mark"><span class="line left"></span><span class="line right"></span></span></div>'};
        if (typeof B === "object") {
            f.extend(C, B)
        } else {
            C.content = B
    ...
    z.success = function (B) {
        var C = {iconDisplay: '<div class="show-zero2 zeromodal-icon zeromodal-success"><span class="line tip"></span><span class="line long"></span><div class="placeholder"></div></div>'};
        if (typeof B === "object") {
            f.extend(C, B)
        } else {
            C.content = B
    ...
    z.confirm = function (D, C) {
        var B = {iconClass: "show-zero2 zeromodal-icon-question", iconText: "?",};
        var E = {};
        f.extend(E, B);
    ...
    function n(D) {
        ...
        if (D.drag) {
            ...
            new a(f('[zero-unique-container="' + D.unique + '"]')[0], {handle: B, limit: false})
        }
        ...
        if (D.transition) {
            f(".zeromodal-container").animate({top: K}, 300)
        }
        p(D);
        if (D.resize) {
            A(D.unique, D)
        ...
        if (!D.url) {
            ...
            if (D.iframe) {
                ...
                E.addClass("zeromodal-overflow-y");
                f.ajax({
                    url: D.url, dataType: "html", type: D.ajaxType, data: D.ajaxData, success: function (M) {
                        E.append(M);
                        f(".zeromodal-loading1").remove();
                        if (typeof D.onComplete === "function") {
                            D.onComplete(D)
